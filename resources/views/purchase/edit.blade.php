      ...
      <div class="rounded-t mb-0 px-0 border-0">
        <div class="px-4 py-2">
          <div class="mt-4 relative w-full max-w-full flex-grow flex-1">
            <h3 class="ml-6 font-semibold text-base text-gray-900 dark:text-gray-50">Editar Compra</h3>
          </div>

          <div class="flex justify-center">
            
            
        <form class="w-1/2 m-9 " action="{{ route('purchase.update', $purchase) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="relative z-0 w-full mb-6 group">
                ...
                <select name="provedor_id" id="provedor_id" data-te-select-init data-te-select-filter="true">
                    ...
                    <option value="{{ $provedor->id }}" @if($purchase->provedor->id == $provedor->id) selected @endif {{ old('provedor_id') == $provedor->id ? 'selected' : '' }}>
                        {{ $provedor->name }}
                    </option>
                    @endforeach
