...
<div class="m-4">
    <div class="relative flex flex-col min-w-0 mb-4 lg:mb-0 break-words bg-gray-50 dark:bg-gray-800 w-full shadow-lg rounded">
      <div class="rounded-t mb-0 px-0 border-0">
        ...
        <form class="w-1/2 m-9 " action="{{ route('brand.update', $brand)  }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            <div class="relative z-0 w-full mb-6 group">
                <input value="{{ old('name',$brand->name) }}" type="text" name="name" id="name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            ...
            <div class="relative z-0 w-full mb-6 group">
                <textarea name="description" id="description" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required >{{ old('description',$brand->description) }}</textarea>
            ...
            <div class="mb-5">
                <input type="hidden" name="picture" value="{{old('picture', $brand->picture)}}" >
                @error('picture')
